...
Please note that for the exponential function the mean is $1/\lambda$ and the standard deviations is $1/\lambda$. 

### Simulation
In order to show the CLT with the exponential function we create $n=40$ samples with $\lambda=0.2$, attach its mean to the sampleMean vector and do this $1000$ times. In the following code. For convenience we store the data the sampleData frame in the last step.
...
```{r,cache=TRUE}
  ...
  # line of sample mean
  geom_vline(aes(xintercept=mean(sampleMeans, na.rm=T)),  color="red", linetype="dashed", size=1) +
  # line of theoretical mean
  geom_vline(aes(xintercept=meanTheor),  color="blue", linetype="dashed", size=1)  
```

However their theoretical and sample mean are so close to eachother it is hard to distinguish them. The theoretical mean is $5.0$ and depending on the simulation the sample mean is somewhat around $5.0$. In most cases the sample mean is around $5.02$ which is realy close to the theoretical mean.
...
Again, the acutual result depends upon the simulation and will be different each time we run the simulatiion, but most of the time the deviation between the two values is between $0.01$ and $0.02$.
...
In the comparison of the two plots, we see that the samples of the exponential function follow obviously a exponential shaped distribution, where the simulated creates a bell shaped histogramm simular to that of a normal distribution.

Furthermore we can use the following Q-Q-Plot to show its approximated normal distribution:
...
Also we can compare the distribution of a large collection of random exponentials and the distribution of a large collection of averages of 40 exponentials, our simulated sample means.
