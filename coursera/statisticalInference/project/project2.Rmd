...
```{r}
  ...
  library(plyr)
  toothSum<-aggregate(ToothGrowth$len, by=list(ToothGrowth$supp,ToothGrowth$dose), FUN=mean)
  toothSum$sd<-aggregate(ToothGrowth$len, by=list(ToothGrowth$supp,ToothGrowth$dose), FUN=sd)$x
  names(toothSum) <- c("supp","dose","meanLength","sdLength")
  toothSum

```

In the summary of the data we can see that in effect the mean value of orange juice is bigger than ascorbic acid, however the standard deviation in the different subsets is higher for orange juice.


###Comparison of tooth growth by supp and dose

Based upon the preliminary analysis we want to examine if the difference in mean between OJ and VC in the same dose level is significant using the t. Also we might be interest interst if a higher dose level has a significatn influence within one supplement. In order to test we assume that a subsets are of unequal variance. $H_0$ states that the difference in mean is equal to zero. $H_1$ states that the difference is mean is not equal to zero.
...
--!>
In order to create all the t-tests we split the data in the different dose levels and the different supplement type. 
```{r,cache=TRUE}
...
```{r,cache=TRUE}
  ...
  c1<-(t.test(dose1OJ$len, dose1VC$len, var.equal=FALSE)); c2<-(t.test(dose2OJ$len, dose2VC$len,var.equal=FALSE))
  c3<-(t.test(dose3OJ$len, dose3VC$len,var.equal=FALSE))
  c4<-(t.test(dose1OJ$len,dose2OJ$len,var.equal=FALSE));c5<-(t.test(dose2OJ$len,dose3OJ$len, var.equal=FALSE))
  c6<-(t.test(dose1OJ$len,dose3OJ$len,var.equal=FALSE))
  c7<-(t.test(dose1VC$len,dose2VC$len,var.equal=FALSE));c8<-(t.test(dose2VC$len,dose3VC$len, var.equal=FALSE))
  c9<-(t.test(dose1VC$len,dose3VC$len,var.equal=FALSE))
  description<-c("d1OJ_vs_d1VC","d2OJ_vs_d2VC","d3OJ_vs_d3VC","d1OJ_vs_d2OJ",
                 "d2OJ_vs_d3OJ","d1OJ_vs_d3OJ","d1VC_vs_d2VC","d2VC_vs_d3VC","d1VC_vs_d3VC")
  pValues<-c(c1$p.value,c2$p.value,c3$p.value,c4$p.value,c5$p.value,c6$p.value,c7$p.value,c8$p.value,c9$p.value)
...
### Conclusion
In conclusion one can say that there is an influence in the mean of the tooth length of guinea pigs between different supplements and different dose levels. However, with a dose level of 2 the difference in mean between OJ and VC is insignificant. 
