...
```{r, echo=FALSE, cache=TRUE}
  ...
  rawData <- read.csv(file="pml-training.csv",na.strings=c("NA","#DIV/0!",""),500)
  subSamples <- createDataPartition(y=rawData$classe, p=3/4, list=FALSE)
  data.train <- trainingset[subSamples, ] 
  data.test <- trainingset[-subSamples, ]
...
```{r, echo=FALSE, cache=TRUE} 
  ...
  data.test.final <- read.csv('pml-testing.csv', na.strings=c("NA","#DIV/0!", ""))
  data.test.final <- preProcessAttributeClass(data.test.final)
  # use only rows in which the training data didn't have a NA values
  data.test.final <- data.test[,colnames(data.train.final)]
  data.test.final.classe <- data.test.final$classe
  data.test.final <- data.test.final[,highlyCorDescr]
  data.test.final$classe <- data.test.final.classe
  predictfinal <- predict(model.rf, data.test.final, type="class")
  predictfinal
